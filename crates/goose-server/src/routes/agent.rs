...
async fn start_agent(
    ...
    // If mcp_only mode is enabled, reject any extension that would spawn a local
    // process; only MCP-over-network (StreamableHttp) extensions are accepted.
    if state.settings.mcp_only {
        for cfg in &extensions_to_use {
